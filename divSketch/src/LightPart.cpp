...
LightPart<init_mem_in_bytes>::LightPart()
{
    ...
    for (int i = 0; i < LIGHT_LEVEL; ++i){
        bobhashs[i] = new BOBHash32(rd() % MAX_PRIME32);
    }
...
//    uint32_t hash_val = (uint32_t)bobhash->run((const char*)key, KEY_LENGTH_4);
    for (int i = 0; i < LIGHT_LEVEL; ++i) {
        BOBHash32 *bobhash = bobhashs[i];
        uint32_t hash_val = (uint32_t)bobhash->run((const char*)key, KEY_LENGTH_4);
        uint32_t pos = hash_val % (uint32_t)counter_num;

        /* insert */
        int old_val = (int)(counters[i][pos]);
        int new_val = (int)(counters[i][pos]) + f;

        new_val = new_val < 255 ? new_val : 255;
        counters[i][pos] = (uint8_t)new_val;
//        if (new_val < 255) {
//            sizes[pos] += size;
//        } else {
//            new_val = 255;
...
int LightPart<init_mem_in_bytes>::query(uint8_t *key)
{
    ...
    for (int i = 0; i < LIGHT_LEVEL; ++i) {
        ...
        uint32_t hash_val = (uint32_t)bobhash->run((const char*)key, KEY_LENGTH_4);
        uint32_t pos = hash_val % (uint32_t)counter_num;
        res = res > (int) counters[i][pos] ? (int) counters[i][pos] : res;
    }
    return res;
}
...
{
    int ns = 0x7FFFFFFF;
    for(int i = 0; i < LIGHT_LEVEL; ++i)  // 遍历d个哈希函数
    {
        int w0 = 0;
        ...
        int temp_ns = (int)(counter_num * (log(counter_num) - log(w0)));
